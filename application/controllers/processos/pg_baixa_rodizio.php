...
$baixa_rod->SetTabelas("processos_rodizios_baixa");
...
$tarefas->SetCondicao(" alerta.id_nome_processo = '9' AND alerta.status = '0' AND alerta.id_usuarios = usuarios.id  AND alerta_item_processo.id = alerta.id_item ");
...
if ($post) {
    ...
    $campos_baixa_ipva = array();
    $campos_baixa_ipva['condutor'] = $post['condutor_nome'];
    $campos_baixa_ipva['requerimento'] = $requerimento;
    $campos_baixa_ipva['laudo_anexo9'] = $laudo_anexo9;
    $campos_baixa_ipva['licenciamento'] = $licenciamento;
    $campos_baixa_ipva['rg_cpf'] = $rg_cpf;
    $campos_baixa_ipva['comprov_end'] = $comprov_end;
    $campos_baixa_ipva['curatela'] = $curatela;
    $campos_baixa_ipva['status'] = $post['status'];
    $campos_baixa_ipva['id_processo'] = $this->uri->segment(4);
    $campos_baixa_ipva['protocolo'] = $post['protocolo'];
    debug($campos_baixa_ipva);

    if (count($result_baixa_rod) == 0) {
        $inserir_baixa_ipva = new Query_model();
        $inserir_baixa_ipva->SetCampos($campos_baixa_ipva);
        $inserir_baixa_ipva->SetTabelas("processos_rodizios_baixa");
        $inserir_baixa_ipva->inserir();
    } else {
        $alterar_baixa_ipva = new Query_model();
        $alterar_baixa_ipva->SetCampos($campos_baixa_ipva);
        $alterar_baixa_ipva->SetTabelas("processos_rodizios_baixa");
